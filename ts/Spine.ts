...
module PhaserSpine {
	...
	export class Spine extends Phaser.Group {
		...
		private getScaleFromVariant(variant: string): number {
			const scale = SpinePlugin.RESOLUTION_REGEXP.exec(variant);
			return scale ? parseFloat(<string>scale[1]) : 1;
		}
		 ...
		 * @returns {any}
		 */
		public setAnimationByName(trackIndex: number, animationName: string, loop: boolean = false): spine.TrackEntry | null {
		 ...
		 * @returns {any}
		 */
		public addAnimationByName(trackIndex: number, animationName: string, loop: boolean = false, delay: number = 0): spine.TrackEntry | null {
			let animation: spine.Animation = this.state.data.skeletonData.findAnimation(animationName);
			if (!animation) {
				console.warn("Animation not found: " + animationName);
				return null;
		 ...
		 * @param skinNames
		 */
		public createCombinedSkin(newSkinName: string, ...skinNames: string[]): spine.Skin | null {
